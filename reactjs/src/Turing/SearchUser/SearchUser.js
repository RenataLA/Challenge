...
class SearchUser extends Component {
  ...
  componentDidMount(){
    var path = this.props.match.params.id == "modify" ? "modify" : "perfil";
  ...
  performSearch = () => {
      fetch('api/search/candidate?table=CANDIDATO&cod='+this.state.filtercod +'&cpf='+this.state.filtercpf+'&email='+this.state.filteremail)
      .then(res => res.json())
      .then(res => {
        var aux = [];
  ...
  renderTableData(){
    return this.state.usersData.map((usersData, index) => {
      ...
      const userLink = `/${path}/${cod_candidato}`
